...
void MultiStageMeanfieldLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype spatial_weight = meanfield_param.spatial_weight();
  Dtype bilateral_weight = meanfield_param.bilateral_weight();
  ...
  if (this->blobs_.size() > 0) {
    ...
    for (int i = 0; i < channels_; i++) {
      this->blobs_[0]->mutable_cpu_data()[i * channels_ + i] = spatial_weight;
    ...
    for (int i = 0; i < channels_; i++) {
      this->blobs_[1]->mutable_cpu_data()[i * channels_ + i] = bilateral_weight;
...
//REGISTER_LAYER_CLASS(MULTI_STAGE_MEANFIELD, MultiStageMeanfieldLayer);
