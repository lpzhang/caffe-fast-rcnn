...
template <typename Dtype>
void KP1SmoothL1LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  // calculate the total numbers of positive objects by summing mask
  Dtype num_pos = 0;
  caffe_gpu_asum(count, bottom[2]->gpu_data(), &num_pos);
...
template <typename Dtype>
void KP1SmoothL1LossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(KP1SmoothL1LossLayer);
